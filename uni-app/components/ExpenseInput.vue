                        ...
                        >
                            ...
                            <div class="form-group">
                                ...
                                <uni-data-select
                                    ...
                                    class="form-select"
                                    required
									:localdata="expenditure_category_dict"
                                >
                                </uni-data-select>
                            </div>
...
const { expenditure_category } = getCache('dict')
const expenditure_category_dict = ref(expenditure_category)
// 定义事件
...
const submitForm = async () => {
	...
	const notPass = expenseList.value.findIndex(x=>!x.itemName || !x.expenseCategory || !x.amount || !x.paymentDate || !x.expensePerson || !x.payee)
	if(notPass != -1){
	...
	for(let i=0;i<expenseList.value.length;i++){
		...
		const isSucc = await addExpenserecords(w).catch(e=>{
			return false
		})
		if(!isSucc){
			isSubmitting.value = true;
